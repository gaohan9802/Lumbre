    ...
    <div className="mx-auto flex min-h-full max-w-7xl flex-col px-4 py-5 sm:px-7 md:h-full">
      ...
      <div className="grid min-h-0 gap-3 md:flex-1 md:grid-cols-[190px_260px_minmax(0,1fr)]">
        ...
        <aside className={`max-h-72 overflow-y-auto rounded-3xl p-3 md:max-h-none ${night ? 'bg-white/[.035]' : 'chat-dialog-card'}`}>
          ...
          <div className="space-y-2">{fieldTopics.map(item => <button key={item.id} onClick={() => setTopicId(item.id)} className={`w-full rounded-2xl px-4 py-3 text-left ${topicId === item.id ? night ? 'bg-[#3a323f]' : 'bg-[#DBB9B3]/25' : 'hover:bg-current/5'}`}><p className="font-serif text-sm leading-5">{item.title}</p><p className="mt-1 text-[10px] opacity-40">{item.entry_count} 条观察</p></button>)}</div>
        </aside>
        <main className={`min-h-[50vh] overflow-y-auto rounded-[30px] border px-5 py-7 sm:px-9 md:min-h-0 ${night ? 'border-white/10 bg-[#192025]' : 'chat-dialog-card'}`}>
          {!topic ? <div className="grid h-full place-items-center text-center opacity-30"><div><BookMarked className="mx-auto mb-3"/><p>从一片田野里打开一个课题。</p></div></div> : <div className="mx-auto max-w-2xl">
            ...
            <div className="relative space-y-7 before:absolute before:bottom-0 before:left-[7px] before:top-2 before:w-px before:bg-current/10">{entries.map(entry => { const [icon, label] = kinds[entry.kind] || kinds.thought; return <article key={entry.id} className="relative pl-8"><span className={`absolute left-0 top-1 grid h-[15px] w-[15px] place-items-center rounded-full text-[11px] ${night ? 'bg-[#28343a]' : 'bg-[#DBB9B3]/25'}`}>{icon}</span><div className="mb-2 flex items-center gap-2 text-[10px] opacity-45"><span>{label}</span><span>·</span><span>{date(entry.created_at)}</span></div>{entry.title && <h3 className="mb-2 font-serif text-lg">{entry.title}</h3>}<p className="whitespace-pre-wrap text-sm leading-7">{entry.content}</p>{entry.source_url && <a href={entry.source_url} target="_blank" rel="noreferrer" className={`mt-3 inline-flex items-center gap-1 text-xs underline decoration-current/30 underline-offset-4 ${night ? 'text-[#708e91]' : 'text-[#9c6e69]'}`}>{entry.source_title || '打开资料来源'}<ExternalLink size={11}/></a>}</article>})}</div>
          </div>}
        </main>
      </div>
