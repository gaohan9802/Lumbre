            ...
            <div className="p-6 space-y-7">
              <section className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setShowKeys(!showKeys)} className="p-2 rounded-lg opacity-60 hover:opacity-100 hover:bg-day-tint dark:hover:bg-white/5">
                ...
                {selectedProvider && (
                  <div className={`p-3 ${subtleCard} space-y-3`}>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <button onClick={() => { if (confirm('删除这个供应商？')) deleteApiProfile(selectedProvider.id) }} className="px-3 py-2 rounded-lg text-xs text-day-error hover:text-day-error dark:text-night-error/80 dark:hover:text-night-error">删除供应商</button>
                    </div>
                    {fetchError && <div className="text-xs text-day-error dark:text-night-error/80 whitespace-pre-wrap">{fetchError}</div>}
              ...
              <section className="pt-4 border-t border-current/10 space-y-2">
                <button onClick={() => { if (confirm('恢复默认设置？API key、供应商和会话都会重置。')) resetSettings() }} className="w-full flex items-center justify-center gap-2 py-2 rounded-xl text-xs opacity-60 hover:opacity-100 hover:bg-day-tint dark:hover:bg-white/5"><RotateCcw size={12} /> 恢复默认</button>
                <button onClick={() => { if (confirm(`清空当前会话 ${messages.length} 条聊天？这不能撤销。`)) clearMessages() }} className="w-full flex items-center justify-center gap-2 py-2 rounded-xl text-xs text-day-error hover:text-day-error dark:text-night-error/80 dark:hover:text-night-error hover:bg-day-errorLight dark:text-night-error/80 dark:hover:text-night-error"><Trash2 size={12} /> 清空当前对话（{messages.length} 条）</button>
              </section>
            </div>
          </motion.div>
        </>
