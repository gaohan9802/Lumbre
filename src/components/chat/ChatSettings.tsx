...
export function ChatSettings(props: Props) {
  ...
  return <>
    ...
    <div ref={dialog} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="chat-menu-title" className={`fixed ${panel === 'star' ? 'inset-x-4 mx-auto top-[8dvh] h-[84dvh] max-w-[640px] rounded-2xl' : 'right-0 top-0 bottom-0 w-[86vw] max-w-[420px]'} z-[71] flex flex-col outline-none ${night ? 'bg-night-surface text-night-text shadow-2xl' : 'chat-paper border-l border-[#a73a32] text-[#3f2c29]'}`}>
      ...
      {panel === 'menu' && <div className={`p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] border-t ${night ? 'border-current/10' : 'border-[#a73a32]/35'}`}><button onClick={() => { continueSession(50); onClose() }} className={`w-full flex items-center justify-center gap-2 rounded-2xl border py-3 text-sm ${night ? 'border-night-amber/25 bg-night-amber/15 text-night-amber' : 'border-[#a73a32] bg-[#fff6df]/70 text-[#9f302b]'}`}><PanelsTopLeft size={16}/>换窗</button></div>}
    </div>
  </>
}
