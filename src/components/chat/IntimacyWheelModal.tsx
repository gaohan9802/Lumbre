...
export function IntimacyWheelModal({ open, night, onClose }: { open: boolean; night: boolean; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/55 backdrop-blur-sm sm:items-center" onClick={onClose}>
    <div role="dialog" aria-modal="true" aria-label="今天怎么操" onClick={event => event.stopPropagation()} className={`max-h-[92dvh] w-full overflow-hidden rounded-t-[30px] shadow-2xl sm:w-[min(620px,calc(100vw-2rem))] sm:rounded-[30px] ${night ? 'bg-[#21151d] text-[#f7e9ef]' : 'bg-[#fff7f8] text-[#512f3f]'}`}>
      ...
      <div className="mx-5 flex rounded-xl bg-black/5 p-1 text-xs"><button onClick={() => setTab('spin')} className={`flex-1 rounded-lg py-2 ${tab === 'spin' ? night ? 'bg-[#714052]' : 'bg-white shadow-sm' : 'opacity-50'}`}>转盘</button><button onClick={() => setTab('manage')} className={`flex-1 rounded-lg py-2 ${tab === 'manage' ? night ? 'bg-[#714052]' : 'bg-white shadow-sm' : 'opacity-50'}`}>元素池</button></div>

      <div className="max-h-[72dvh] overflow-y-auto px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-4">
        {tab === 'spin' ? <>
          <div className="mb-4 flex flex-wrap gap-2">{pools.map(pool => <button key={pool.id} aria-pressed={selected.has(pool.id)} onClick={() => setSelected(current => { const next = new Set(current); next.has(pool.id) ? next.delete(pool.id) : next.add(pool.id); return next })} className={`rounded-full border px-3 py-1.5 text-xs transition ${selected.has(pool.id) ? 'border-[#ad596e] bg-[#ad596e] text-white' : 'border-current/15 opacity-45'}`}>{pool.emoji} {pool.name}</button>)}</div>

          <div className="grid min-h-52 grid-cols-2 gap-2 sm:grid-cols-3">{results.length ? results.map(item => <div key={item.pool_id} className={`relative rounded-2xl border p-3 text-left transition ${pinned.has(item.pool_id) ? 'border-[#d59a72] bg-[#d59a72]/10' : 'border-current/10 bg-black/[.025]'}`}><span className="text-[10px] opacity-45">{item.pool}</span><p className="mt-1 pr-8 text-sm leading-5">{item.text}</p><div className="absolute right-2 top-2 flex gap-1"><button aria-label={`只重转${item.pool}`} onClick={() => { setPinned(current => { const next = new Set(current); next.delete(item.pool_id); return next }); void spin([item.pool_id]) }} className="p-1 opacity-40 hover:opacity-100"><RotateCcw size={12}/></button><button aria-label={`固定${item.pool}`} onClick={() => setPinned(current => { const next = new Set(current); next.has(item.pool_id) ? next.delete(item.pool_id) : next.add(item.pool_id); return next })} className={`p-1 ${pinned.has(item.pool_id) ? 'text-[#d59a72]' : 'opacity-40'}`}><Pin size={12} fill={pinned.has(item.pool_id) ? 'currentColor' : 'none'}/></button></div></div>) : <div className="col-span-full flex flex-col items-center justify-center opacity-35"><Dices size={42} strokeWidth={1.2}/><p className="mt-3 text-sm">选好想玩的池，然后转</p></div>}</div>

          {error && <p className="mt-3 text-center text-xs text-red-400">{error}</p>}
          <div className="mt-5 grid grid-cols-[1fr_auto] gap-2"><button disabled={busy || !chosen.length} onClick={() => void spin()} className="rounded-2xl bg-gradient-to-r from-[#873f58] to-[#bd6d78] py-3.5 text-lg font-semibold text-white shadow-lg disabled:opacity-30">转</button><button disabled={!results.length} onClick={share} className="flex items-center gap-1 rounded-2xl border border-current/15 px-4 text-xs disabled:opacity-25"><Send size={15}/>发给星星</button></div>
          {!!pinned.size && <p className="mt-2 text-center text-[10px] opacity-45">点结果可以固定；再点「转」只重转没固定的池</p>}
          {!!recent.length && <details className="mt-4 text-xs opacity-50"><summary className="cursor-pointer list-none">最近转过 · {recent.length}</summary><div className="mt-2 max-h-32 space-y-2 overflow-y-auto">{recent.slice(0, 10).map(item => <button key={item.id} onClick={() => { setResults(item.results); setPinned(new Set()) }} className="block w-full rounded-xl bg-black/5 px-3 py-2 text-left"><span>{new Date(item.at).toLocaleString('zh-CN', { timeZone: 'Europe/Madrid', month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span><br/><span className="opacity-70">{item.results.map(result => result.text).join(' · ')}</span></button>)}</div></details>}
        </> : <div className="space-y-2">
          {pools.map(pool => <section key={pool.id} className="overflow-hidden rounded-2xl border border-current/10">
            ...
            {expanded === pool.id && <div className="border-t border-current/10 px-3 py-3">
              <div className="mb-3 flex gap-2"><input value={newText} onChange={event => setNewText(event.target.value)} placeholder="加一个新元素" className="min-w-0 flex-1 rounded-xl border border-current/10 bg-transparent px-3 py-2 text-sm outline-none"/><button disabled={!newText.trim() || busy} onClick={async () => { if (await act('add', { pool_id: pool.id, text: newText })) setNewText('') }} className="rounded-xl bg-[#ad596e] px-3 text-white"><Plus size={16}/></button></div>
