  ...
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/55 backdrop-blur-sm sm:items-center" onClick={onClose}>
    <div role="dialog" aria-modal="true" aria-label="今天怎么操" onClick={event => event.stopPropagation()} className={`max-h-[92dvh] w-full overflow-hidden rounded-t-[30px] sm:w-[min(620px,calc(100vw-2rem))] sm:rounded-[30px] ${night ? 'border border-night-border bg-night-surface text-night-text shadow-2xl' : 'chat-dialog'}`}>
      ...
      <div className="max-h-[72dvh] overflow-y-auto px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-4">
        {tab === 'spin' ? <>
          <div className="mb-4 flex flex-wrap gap-2">{pools.map(pool => <button key={pool.id} aria-pressed={selected.has(pool.id)} onClick={() => setSelected(current => { const next = new Set(current); next.has(pool.id) ? next.delete(pool.id) : next.add(pool.id); return next })} className={`rounded-full border px-3 py-1.5 text-xs transition ${selected.has(pool.id) ? night ? 'border-night-amber bg-night-amber text-night-bg' : 'chat-dialog-accent' : 'border-current/15 opacity-45'}`}>{pool.emoji} {pool.name}</button>)}</div>

          <div className="grid min-h-52 grid-cols-2 gap-2 sm:grid-cols-3">{results.length ? results.map(item => <div key={item.pool_id} className={`relative rounded-2xl border p-3 text-left transition ${pinned.has(item.pool_id) ? night ? 'border-night-amber/55 bg-night-amber/10' : 'chat-dialog-accent' : night ? 'border-night-border bg-night-card/55' : 'chat-dialog-card'}`}><span className="text-[10px] opacity-45">{item.pool}</span><p className="mt-1 pr-8 text-sm leading-5">{item.text}</p><div className="absolute right-2 top-2 flex gap-1"><button aria-label={`只重转${item.pool}`} onClick={() => { setPinned(current => { const next = new Set(current); next.delete(item.pool_id); return next }); void spin([item.pool_id]) }} className="p-1 opacity-40 hover:opacity-100"><RotateCcw size={12}/></button><button aria-label={`固定${item.pool}`} onClick={() => setPinned(current => { const next = new Set(current); next.has(item.pool_id) ? next.delete(item.pool_id) : next.add(item.pool_id); return next })} className={`p-1 ${pinned.has(item.pool_id) ? night ? 'text-night-amber' : 'text-[#9c6e69]' : 'opacity-40'}`}><Pin size={12} fill={pinned.has(item.pool_id) ? 'currentColor' : 'none'}/></button></div></div>) : <div className="col-span-full flex flex-col items-center justify-center opacity-35"><Dices size={42} strokeWidth={1.2}/><p className="mt-3 text-sm">选好想玩的池，然后转</p></div>}</div>
