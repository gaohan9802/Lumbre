...
export function StoriesView() {
  ...
  return (
    <div className={`h-full overflow-hidden ${night ? 'bg-[#12131a] text-[#eee8df]' : 'chat-paper text-[#3f2c29]'}`}>
      <div className="mx-auto flex h-full max-w-6xl flex-col px-4 py-5 sm:px-7">
        <header className="mb-4 flex flex-wrap items-end justify-between gap-3">
          ...
          <button onClick={create} disabled={busy} className={`rounded-full px-4 py-2 text-xs disabled:opacity-40 ${night ? 'bg-[#8e5263] text-white' : 'chat-dialog-accent'}`}><Plus className="mr-1 inline" size={14}/>新故事</button>
        </header>
        <div className="mb-4 flex items-center gap-2">
          {(['moonlight', 'undertow'] as Shelf[]).map(value => <button key={value} onClick={() => setShelf(value)} className={`rounded-full px-4 py-2 text-xs ${shelf === value ? (night ? (value === 'moonlight' ? 'bg-[#7d789d] text-white' : 'bg-[#7d4658] text-white') : 'chat-dialog-accent') : 'bg-current/5'}`}>{value === 'moonlight' ? '☾ 月灯' : '◐ 暗潮'}</button>)}
      ...
      {editing && story && <div className="fixed inset-0 z-[120] grid place-items-center bg-black/60 p-4 backdrop-blur-sm">
        <div className={`flex max-h-[92dvh] w-full max-w-3xl flex-col rounded-[28px] p-5 shadow-2xl ${night ? 'bg-[#20212b]' : 'chat-dialog'}`}>
          ...
          <div className="mb-3 flex gap-2">{(['moonlight', 'undertow'] as Shelf[]).map(value => <button key={value} onClick={() => setDraft(item => ({ ...item, shelf: value }))} className={`rounded-full px-3 py-1.5 text-xs ${draft.shelf === value ? (night ? 'bg-[#8e5263] text-white' : 'chat-dialog-accent') : 'bg-current/5'}`}>{shelfName(value)}</button>)}</div>
          ...
          <button disabled={busy || !draft.title.trim()} onClick={save} className={`mt-4 rounded-2xl py-3 text-sm disabled:opacity-40 ${night ? 'bg-[#8e5263] text-white' : 'chat-dialog-accent'}`}>保存故事</button>
        </div>
      </div>}
    </div>
  )
