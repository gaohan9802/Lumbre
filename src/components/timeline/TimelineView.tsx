...
export function TimelineView(){
  ...
  useEffect(()=>{
    ...
    const schedule = () => {
      ...
      timer = setTimeout(() => {
        const pool = encouragements.filter(x => x.enabled && (x.scope === 'permanent' || x.tags?.some((v:string) => current.tags.includes(v))))
        if (pool.length) {
          setBubble(prev => {
  ...
  return <div className={`h-full overflow-y-auto ${n?'bg-night-bg text-night-text':'bg-day-bg text-day-text'}`}>
    <div className="max-w-6xl mx-auto px-4 md:px-7 py-6 pb-24">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6"><div><h1 className="font-serif text-2xl">Timeline</h1><p className={`text-xs mt-1 ${n?'text-night-muted':'text-day-muted'}`}>一天不是被切碎的，是一段一段活过来的。</p></div><div className="flex items-center gap-2"><button onClick={()=>setTagEditor(true)} className={`p-2.5 rounded-xl border ${n?'border-night-border hover:bg-night-surface':'border-day-border hover:bg-day-bg'}`} title="管理标签和鼓励话"><Settings2 size={16}/></button><button onClick={()=>setTimerOpen(true)} className={`px-4 py-2.5 rounded-xl flex items-center gap-2 ${n?'bg-night-amber text-night-bg':'bg-day-pink text-white'}`}><Clock3 size={15}/>{current?'查看当前状态':'开始一件事'}</button></div></div>
