...
export function TimelineView(){
  ...
  return <div className={`h-full overflow-y-auto ${n?'bg-night-bg text-night-text':'chat-paper text-[#3f2c29]'}`}>
    <div className="max-w-6xl mx-auto px-4 md:px-7 py-6 pb-24">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6 px-1"><div><p className={`text-[10px] tracking-[0.3em] mb-1 ${n?'text-night-muted':'text-[#8a6f69]'}`}>TIME AT HOME</p><h1 className="text-2xl font-bold">Timeline</h1></div><div className="flex items-center gap-2"><button onClick={()=>setTagEditor(true)} className={`p-2.5 rounded-xl border ${n?'border-night-border hover:bg-night-surface':'chat-dialog-line hover:bg-[#DBB9B3]/20'}`} title="管理标签和鼓励话"><Settings2 size={16}/></button><button onClick={()=>setTimerOpen(true)} className={`px-4 py-2.5 rounded-xl flex items-center gap-2 ${n?'bg-night-amber text-night-bg':'chat-dialog-accent'}`}><Clock3 size={15}/>{current?'查看当前状态':'开始一件事'}</button></div></div>
      {current&&<button onClick={()=>setTimerOpen(true)} className={`w-full mb-5 p-4 rounded-2xl text-left ${n?'border border-night-amber/25 bg-night-card':'chat-dialog-card'}`}><div className="text-[10px] opacity-50">此刻正在</div><div className="font-medium mt-1">{current.title}</div><div className="text-xs opacity-55 mt-1">从 {fmtTime(current.start_at)} 开始 · 点击结束</div></button>}
      {current && <section className={`mb-5 rounded-2xl p-5 text-center ${n?'border border-night-border bg-night-card':'chat-dialog-card'}`}>
        <div className="text-[10px] uppercase tracking-[0.2em] opacity-45 mb-3">正在专注 · {current.tags.join(' / ')}</div>
        <div className={`mx-auto w-56 h-56 rounded-full grid place-items-center border-[10px] ${n?'border-night-amber/70':'border-[#DBB9B3]/70'}`}><div><div className="font-mono text-4xl md:text-5xl tracking-wider">{(()=>{const sec=Math.max(0,Math.floor((now-new Date(current.start_at).getTime())/1000));return `${String(Math.floor(sec/3600)).padStart(2,'0')}:${String(Math.floor(sec%3600/60)).padStart(2,'0')}:${String(sec%60).padStart(2,'0')}`})()}</div></div></div>
        {bubble.length>0 && <div className="relative mx-auto mt-5 max-w-md space-y-2">{bubble.map((item,i)=><div key={item._key||item.id} className={`relative rounded-2xl px-4 py-3 text-sm text-left transition-all ${i===0?'':'opacity-75 scale-[.98]'} ${n?'bg-night-surface':'bg-[#DBB9B3]/25'}`}><Sparkles size={13} className="absolute left-3 top-3 opacity-60"/><span className="block px-4">{item.text}</span></div>)}</div>}
      </section>}
      <div className="flex items-center justify-center gap-3 mb-5"><button onClick={()=>move(-1)} className="p-2 rounded-full hover:bg-black/5"><ChevronLeft size={18}/></button><input type="date" value={date} onChange={e=>setDate(e.target.value)} className={`no-frame rounded-xl border px-3 py-2 text-sm ${n?'bg-night-card border-night-border':'chat-dialog-field'}`}/><button onClick={()=>setDate(dayKey(new Date()))} className="text-xs opacity-60 hover:opacity-100">今天</button><button onClick={()=>move(1)} className="p-2 rounded-full hover:bg-black/5"><ChevronRight size={18}/></button></div>
    ...
    {tagEditor&&<><div className="fixed inset-0 z-[80] bg-black/45 backdrop-blur-sm" onClick={()=>setTagEditor(false)}/><div className={`fixed z-[81] inset-x-4 mx-auto top-[7dvh] max-w-2xl max-h-[86dvh] overflow-y-auto rounded-[1.6rem] p-5 ${n?'border border-night-border bg-night-surface text-night-text shadow-2xl':'chat-dialog'}`}>
      ...
      <div className={`flex gap-1 p-1 rounded-xl mb-5 ${n?'bg-night-bg':'bg-[#DBB9B3]/15'}`}><button onClick={()=>setSettingsTab('tags')} className={`flex-1 rounded-lg py-2 text-xs ${settingsTab==='tags'?(n?'bg-night-amber text-night-bg':'chat-dialog-accent'):''}`}>标签管理</button><button onClick={()=>setSettingsTab('encouragements')} className={`flex-1 rounded-lg py-2 text-xs ${settingsTab==='encouragements'?(n?'bg-night-amber text-night-bg':'chat-dialog-accent'):''}`}>鼓励话管理</button></div>
      {settingsTab === 'tags' ? <div className="space-y-4">
        <div><p className="mb-2 text-xs font-medium">活动标签</p><div className="flex flex-wrap gap-2">{tags.map(tag => <span key={tag} className={`inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-xs ${n ? 'bg-night-bg' : 'bg-[#DBB9B3]/18'}`}>{tag}<button onClick={() => setTags(tags.filter(x => x !== tag))} disabled={tags.length <= 1} className="opacity-50 hover:opacity-100 disabled:opacity-20"><X size={12}/></button></span>)}</div></div>
        <div className="flex gap-2"><input value={tagDraft} onChange={e => setTagDraft(e.target.value)} onKeyDown={e => e.key === 'Enter' && addTag()} placeholder="添加一个标签" className={`no-frame flex-1 rounded-xl border px-3 py-2 text-sm ${n ? 'bg-night-bg border-night-border' : 'chat-dialog-field'}`}/><button onClick={addTag} className={`rounded-xl px-4 ${n ? 'bg-night-amber text-night-bg' : 'chat-dialog-accent'}`}><Plus size={16}/></button></div>
        <button disabled={settingsBusy} onClick={saveTags} className={`w-full rounded-xl py-3 ${n ? 'bg-night-amber text-night-bg' : 'chat-dialog-accent'}`}>{settingsBusy ? '保存中…' : '保存标签'}</button>
      </div> : <div className="space-y-4">
        <div className={`rounded-2xl p-4 ${n ? 'border border-night-border bg-night-bg/40' : 'chat-dialog-card'}`}>
          <p className="mb-3 text-xs font-medium">{editingEnc ? '编辑鼓励话' : '添加鼓励话'}</p>
          <textarea value={encDraft.text} onChange={e => setEncDraft({...encDraft, text:e.target.value})} rows={3} placeholder="例如：慢一点也没关系，你正在靠近想去的地方。" className={`no-frame w-full resize-none rounded-xl border px-3 py-2 text-sm ${n ? 'bg-night-bg border-night-border' : 'chat-dialog-field'}`}/>
          <div className="mt-3 flex gap-2"><button onClick={() => setEncDraft(v => ({...v, scope:'permanent'}))} className={`rounded-full border px-3 py-1.5 text-xs ${encDraft.scope === 'permanent' ? (n ? 'border-night-amber text-night-amber' : 'chat-dialog-accent') : ''}`}>常驻</button><button onClick={() => setEncDraft(v => ({...v, scope:'tags'}))} className={`rounded-full border px-3 py-1.5 text-xs ${encDraft.scope === 'tags' ? (n ? 'border-night-amber text-night-amber' : 'chat-dialog-accent') : ''}`}>按标签</button></div>
          {encDraft.scope === 'tags' && <div className="mt-3 flex flex-wrap gap-2">{tags.map(tag => <button key={tag} onClick={() => toggleEncTag(tag)} className={`rounded-full border px-3 py-1.5 text-xs ${encDraft.tags.includes(tag) ? (n ? 'border-night-amber bg-night-amber/15 text-night-amber' : 'chat-dialog-accent') : ''}`}>{tag}</button>)}</div>}
          <div className="mt-4 flex gap-2"><button disabled={settingsBusy} onClick={saveEnc} className={`flex-1 rounded-xl py-2.5 ${n ? 'bg-night-amber text-night-bg' : 'chat-dialog-accent'}`}>{settingsBusy ? '保存中…' : editingEnc ? '保存修改' : '添加鼓励话'}</button>{editingEnc && <button onClick={() => {setEditingEnc(null);setEncDraft({text:'',scope:'permanent',tags:[]})}} className={`rounded-xl border px-3 ${n ? 'border-night-border' : 'chat-dialog-line'}`}><RotateCcw size={15}/></button>}</div>
        </div>
        <div className="space-y-2">{encouragements.map(x => <div key={x.id} className={`rounded-2xl p-3 ${n ? 'border border-night-border' : 'chat-dialog-card'}`}><div className="flex items-start gap-3"><div className="flex-1 text-sm leading-relaxed">{x.text}<div className="mt-2 text-[10px] opacity-45">{x.scope === 'permanent' ? '常驻' : `#${(x.tags || []).join(' #')}`} · {x.enabled ? '启用' : '已停用'}</div></div><button onClick={() => toggleEnc(x)} title={x.enabled ? '停用' : '启用'} className={`p-1.5 ${x.enabled ? 'text-day-success' : 'opacity-40'}`}><Check size={14}/></button><button onClick={() => editEnc(x)} className="p-1.5 opacity-50 hover:opacity-100"><Pencil size={14}/></button><button onClick={() => deleteEnc(x.id)} className="p-1.5 text-day-error opacity-60 hover:opacity-100"><Trash2 size={14}/></button></div></div>)}</div>
      </div>}
    </div></>}

    {editing&&<><div className="fixed inset-0 z-[80] bg-black/45" onClick={()=>setEditing(null)}/><div className={`fixed z-[81] inset-x-4 mx-auto top-[10dvh] max-w-lg rounded-3xl p-5 max-h-[80dvh] overflow-y-auto ${n?'border border-night-border bg-night-surface shadow-2xl':'chat-dialog'}`}><div className="flex justify-between mb-4"><h3 className="font-medium">编辑记录</h3><button onClick={()=>setEditing(null)}><X size={17}/></button></div><div className="space-y-3 text-xs"><label className="block">事情<input value={editing.title} onChange={e=>setEditing({...editing,title:e.target.value})} className={`no-frame mt-1 w-full rounded-xl border px-3 py-2 ${n?'bg-night-bg border-night-border':'chat-dialog-field'}`}/></label><label className="block">标签<input value={editing.tags.join(', ')} onChange={e=>setEditing({...editing,tags:e.target.value.split(/[,，\s]+/).filter(Boolean)})} className={`no-frame mt-1 w-full rounded-xl border px-3 py-2 ${n?'bg-night-bg border-night-border':'chat-dialog-field'}`}/></label><div className="grid grid-cols-2 gap-2"><label>开始<input type="datetime-local" value={formatMadridInput(editing.start_at)} onChange={e=>setEditing({...editing,start_at:e.target.value})} className={`no-frame mt-1 w-full rounded-xl border px-2 py-2 ${n?'bg-night-bg border-night-border':'chat-dialog-field'}`}/></label><label>结束<input type="datetime-local" value={editing.end_at?formatMadridInput(editing.end_at):''} onChange={e=>setEditing({...editing,end_at:e.target.value||undefined})} className={`no-frame mt-1 w-full rounded-xl border px-2 py-2 ${n?'bg-night-bg border-night-border':'chat-dialog-field'}`}/></label></div><label className="block">开始备注<textarea value={editing.note||''} onChange={e=>setEditing({...editing,note:e.target.value})} rows={3} className={`no-frame mt-1 w-full rounded-xl border px-3 py-2 ${n?'bg-night-bg border-night-border':'chat-dialog-field'}`}/></label><label className="block">结束备注<textarea value={editing.end_note||''} onChange={e=>setEditing({...editing,end_note:e.target.value})} rows={3} className={`no-frame mt-1 w-full rounded-xl border px-3 py-2 ${n?'bg-night-bg border-night-border':'chat-dialog-field'}`}/></label><button onClick={saveEdit} className={`w-full py-3 rounded-xl flex justify-center items-center gap-2 ${n?'bg-night-amber text-night-bg':'chat-dialog-accent'}`}><Save size={14}/>保存修改</button></div></div></>}
  </div>
}
