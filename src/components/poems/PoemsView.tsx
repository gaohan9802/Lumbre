...
export function PoemsView() {
  ...
  if (!poem) return (
    <div className={`h-full overflow-y-auto px-6 py-10 ${night ? 'bg-[#171512] text-[#eee5d8]' : 'bg-[#f7f0e7] text-[#57483f]'}`}>
      <div className="mx-auto max-w-2xl text-center"><Feather className="mx-auto mb-4 opacity-50" /><h1 className="font-serif text-3xl">共诗</h1><p className="mt-2 text-sm opacity-60">你一句，我一句。</p><button disabled={busy} onClick={create} className="mt-8 rounded-full bg-[#9d5361] px-5 py-2.5 text-sm text-white"><Plus className="mr-1 inline" size={15}/>新建一首诗</button>{error && <p className="mt-4 text-sm text-red-500">{error}</p>}</div>
    </div>
  )

  return (
    <div className={`h-full overflow-y-auto ${night ? 'bg-[#171512] text-[#eee5d8]' : 'bg-[#f7f0e7] text-[#57483f]'}`}>
      <div className="mx-auto max-w-3xl px-4 py-6 sm:px-8">
        <div className="mb-5 flex items-center gap-2 overflow-x-auto pb-2">
          {poems.map(item => <button key={item.id} onClick={() => setSelected(item.id)} className={`shrink-0 rounded-full px-3 py-1.5 text-xs ${item.id === poem.id ? 'bg-[#9d5361] text-white' : night ? 'bg-white/5' : 'bg-white/70'}`}>{item.archived ? '📦 ' : ''}{item.title}</button>)}
          <button aria-label="新建一首诗" onClick={create} className={`shrink-0 rounded-full p-2 ${night ? 'bg-white/5' : 'bg-white/70'}`}><Plus size={14}/></button>
        ...
        <article className={`relative overflow-hidden rounded-[28px] border px-5 py-8 shadow-sm sm:px-10 ${night ? 'border-white/10 bg-[#211e1a]' : 'border-[#dfcfc0] bg-[#fffaf3]'}`}>
          ...
          <div className="relative mt-10 border-t border-current/10 pt-5">
            {poem.archived ? <button onClick={() => void act('archive', { id: poem.id, archived: false })} className="w-full rounded-2xl border border-current/15 px-4 py-3 text-sm">把它放回诗笺</button> : poem.turn === currentUser ? <div className="flex gap-2"><textarea value={draft} onChange={event => setDraft(event.target.value)} rows={2} placeholder="写下一句……" className={`min-w-0 flex-1 resize-none rounded-2xl border bg-transparent px-4 py-3 font-serif outline-none ${night ? 'border-white/10' : 'border-[#dfcfc0]'}`}/><button disabled={busy || !draft.trim()} onClick={append} className="rounded-2xl bg-[#9d5361] px-4 text-white disabled:opacity-30"><Feather size={18}/></button></div> : <button onClick={() => share(true)} className="w-full rounded-2xl bg-[#9d5361] px-4 py-3 text-sm text-white"><MessageCircle className="mr-2 inline" size={16}/>叫星星接一句</button>}
