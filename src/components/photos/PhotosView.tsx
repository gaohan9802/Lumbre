      ...
      <div className="max-w-lg mx-auto px-6 py-6 space-y-6">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setZone('public')}
            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs transition ${zone === 'public' ? (isNight ? 'bg-night-amber/20 text-night-amber' : 'bg-[#DBB9B3]/25 text-[#765953]') : 'opacity-50'}`}>
            <Eye size={12} /> 公开区
          </button>
          <button onClick={() => setZone('locked')}
            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs transition ${zone === 'locked' ? (isNight ? 'bg-night-amber/20 text-night-amber' : 'bg-[#DBB9B3]/25 text-[#765953]') : 'opacity-50'}`}>
            <Lock size={12} /> 上锁区
          </button>
          {zone === 'locked' && (
        ...
        <AnimatePresence>
          {settingPassword && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
              className={`p-3 rounded-xl ${isNight ? 'bg-night-surface' : 'chat-dialog-card'}`}>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={savePassword} className={`px-3 py-2 rounded-lg text-xs ${isNight ? 'bg-night-amber text-night-bg' : 'chat-dialog-accent'}`}>保存</button>
            ...
            <div className="grid grid-cols-2 gap-3">
              {photos.map((p) => (
                <motion.button key={p.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  className={`text-left rounded-2xl overflow-hidden ${isNight ? 'bg-night-surface' : 'chat-dialog-card'}`}>
        ...
        {active && (
          <>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              className={`fixed bottom-0 left-0 right-0 z-[71] max-h-[88dvh] rounded-t-2xl overflow-y-auto ${isNight ? 'bg-night-card text-night-text' : 'chat-dialog'}`}
              ...
              <div className="flex items-center justify-between p-3 sticky top-0 backdrop-blur-md">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => setActive(null)} className={`p-2 rounded-xl opacity-70 hover:opacity-100 ${isNight ? 'bg-night-surface' : 'bg-[#DBB9B3]/20'}`}><X size={16} /></button>
                </div>
              </div>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              ...
              <div className="p-4 space-y-4">
                  ...
                  {editingCaption ? (
                    <div className="flex items-start gap-2">
                      <textarea value={captionDraft} onChange={(e) => setCaptionDraft(e.target.value)} rows={2} autoFocus
                        className={`flex-1 text-sm p-2 rounded-xl outline-none resize-none ${isNight ? 'bg-night-surface' : 'chat-dialog-field'}`} />
                      <button onClick={saveCaption} className={`p-2 rounded-xl ${isNight ? 'bg-night-amber text-night-bg' : 'chat-dialog-accent'}`}><Check size={16} /></button>
                    </div>
                  ) : (
                    <div className="flex items-start gap-2">
                ...
                <div className="space-y-2">
                  {active.comments?.map((c, i) => (
                    <div key={i} className={`text-xs p-2 rounded-xl ${isNight ? 'bg-night-surface' : 'bg-[#DBB9B3]/15'}`}>
                      <span className="opacity-50 mr-1">{emojiFor(c.author)}</span>
                      <span>{c.content}</span>
                      <span className="opacity-30 ml-2 text-[10px]">{fmt(c.time)}</span>
                    </div>
                  ))}
                  <div className={`flex items-center gap-2 px-3 py-2 rounded-xl ${isNight ? 'bg-night-surface' : 'chat-dialog-field'}`}>
        ...
        {deleteConfirm && (
          <>
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[81] w-[280px] p-6 rounded-2xl text-center ${isNight ? 'bg-night-card' : 'chat-dialog'}`}>
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setDeleteConfirm(null)}
                  className={`flex-1 py-2 rounded-xl text-xs ${isNight ? 'bg-night-surface' : 'bg-[#DBB9B3]/15'}`}>取消</button>
