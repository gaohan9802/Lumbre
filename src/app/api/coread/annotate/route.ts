...
export async function POST(req: NextRequest) {
  try {
    ...
    const { action, bookId, chapterNum, originalText, annotation, annId } = body
    ...
    // Default: add
    if (!bookId || chapterNum === undefined || !annotation) {
      return NextResponse.json({ error: 'missing params' }, { status: 400 })
    }
    const ann = addAnnotation(bookId, chapterNum, originalText || '', annotation, 'user')
