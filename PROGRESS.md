...
## 2026-08-07 — 星星气泡与工具调用改为左对齐
- 星星历史正文、content blocks、流式正文及加载气泡统一改为靠左，继续使用内容宽度且最大为消息区 80%，不再占满屏幕。
- content block、legacy 与流式三条工具调用渲染路径同步靠左，并限制为与星星气泡一致的最大 80% 宽度；思考链的位置与样式保持不变。
- user 消息下方的重试、删除、复制、修改操作区统一靠右；删除版本菜单也从 user 操作区右侧展开。星星操作区继续靠左。
- 验证：`tsc --noEmit`、`git diff --check` 均通过；未运行 Next production build。

## 2026-08-07 — 星星气泡加宽与正文节奏收紧
- 星星正文、工具调用、流式正文及加载气泡的最大宽度由消息区 80% 调整为 87%，继续保持靠左与内容自适应；user 气泡仍为 80%。
...
## 2026-07-16 — 摘要系统倒序整理与会话级配置

### 完成
- 摘要改为从当前对话末尾向前倒序分段：第一张先整理最近 N 轮；若已有多段积压，后续自动继续向更早区间回填，不再从会话开头正序开始。
- “轮”按 user 消息划分，包含其后的 assistant 回复及中间工具过程；不再假设每轮严格等于两条消息。
- 摘要弹窗显示当前最新未整理轮数，以及距离自动生成还差多少轮/至少多少条消息。
- 自动整理开关、分段轮数、注入张数、摘要 API、摘要模型全部成为 `ChatSession.summaryConfig`，每个对话独立保存和同步。
- 摘要模型与聊天模型解耦，可单独选择任一已配置 API profile 和模型。
- 前端仅展示离当前最近的 5 张摘要；其余摘要仍随完整 session 永久保存在 `/persistent/chat/sessions/`，并显示归档数量。
- 摘要卡显示实际覆盖的轮数与消息条数。
- 新摘要保存 `sourceMessageIds`，删除消息、截断、分支时可精确判断哪些摘要失效；兼容旧摘要的时间区间判定。
- 自动整理关闭后不再触发；重新打开或修改阈值后，如已有足够积压会继续倒序整理。

### 验证
- `tsc --noEmit` 通过。
- `git diff --check` 通过。
- 45 轮倒序断言通过：先取第 26–45 轮，再取第 6–25 轮。
- 未运行 Next production build，继续遵守低内存 shell 策略。

---

## 2026-07-16 — 摘要纠错、结构化记忆、阶段摘要与折叠 UI

### 完成
1. **普通摘要可纠错**
   - 每张摘要支持手动编辑事件摘要、双方情绪及全部结构化字段
   - 支持“摘要有误”标记，卡片显示红色纠错状态
   - 支持锁定/解锁；锁定后禁止模型重新生成
   - 支持按原始来源消息重新生成单张摘要，成功后清除纠错标记
2. **摘要结构化**
   - 新增：确认事实、约定与承诺、偏好变化、未完成事项、需要避免、关系状态变化、后续追踪
   - 新摘要生成提示词改为严格 JSON；旧摘要保持兼容，缺少结构字段时仍可正常显示和编辑
   - 普通摘要注入聊天上下文时同步带入结构化信息
3. **阶段摘要**
   - 每累计 10 张尚未归入阶段的普通摘要，自动生成 1 张阶段摘要
   - 阶段摘要包含标题、阶段总览和相同的结构化字段
   - 原始普通摘要永久保留，不做删除或覆盖
   - 聊天上下文注入最近 2 张阶段摘要，再注入当前会话配置的最近 3/4/5 张普通摘要
4. **前端折叠**
   - 普通摘要和阶段摘要默认折叠，只显示日期、覆盖量、状态和单行预览
   - 点击卡片标题/箭头后展开全文，避免摘要弹窗过长
5. **一致性处理**
   - 普通摘要内容被编辑、重生成、删除，或来源消息失效时，对应阶段摘要自动失效，随后可按新内容重建
   - 锁定、纠错状态切换不改变摘要正文，因此不会无意义地重建阶段摘要
   - 阶段摘要请求失败后对同一批次做页面生命周期内去重，避免反复消耗 API

### 验证
- `git diff --check`：通过
- TypeScript：使用现有完整依赖目录进行 `tsc --noEmit`，本次改动无类型错误；仅项目环境原有可选依赖 `web-push` 类型缺失阻断全量检查
- 未运行 Next production build，避免本机 OOM，交由 Zeabur 自动构建

---
## 2026-08-05 — 摘要系统简化与旧历史封存

### 完成
- 将普通摘要精简为唯一正文 `eventSummary`；移除确认事实、约定承诺、偏好、未完成事项、避免事项、关系变化、后续追踪，以及小火/星星情绪字段。
- 完整删除阶段摘要的生成、存储注入和界面；聊天只注入最近几张单块正文摘要。
- 摘要生成 prompt 完整替换为用户提供的「麻糍」SEGMENT_PROMPT，要求第二人称、按日期、300–800 字、忠实保留关键事件/约定/感官与原文尺度，只输出正文。
- 摘要请求独立到 `/api/chat/summary`，复用当前会话选择的 provider/model，但不开放聊天工具，避免摘要模型误调用工具。
- 为每个会话加入摘要 v2 封存游标。升级时把当前最后一层设为 anchor，已有 4000 多层对话全部视为封存历史，不再自动倒序补整理。
- 关闭自动整理后，重新开启会把 anchor 推到开启当下；关闭期间内容永久跳过，不会形成待补队列。
- 自动整理仍按会话阈值工作；手动按钮可整理当前 anchor 后的新对话。摘要成功后才推进 anchor。
- 摘要弹窗改为纯正文折叠卡，保留编辑与删除；删除结构化编辑、纠错标记、锁定、重生成与阶段卡。
- 自动整理开关改为 56×32 固定滑轨、内边距约束和 Framer Motion spring 滑块，修复视觉漂移并统一日夜配色。
- 旧结构化摘要迁移时只提取正文；旧阶段摘要不再读取。

### 验证
- TypeScript 检查除项目已有可选依赖 `web-push` 类型缺失外，本次改动无报错。
- `git diff --check` 通过。
- 未在低内存 shell 执行 Next production build，交由 Zeabur 自动构建。

---

## 2026-07-06 — 恢复摘要高级能力（保留纯正文方案）

### 完成
- 恢复阶段摘要：每累计 10 张尚未压缩的普通摘要，自动生成 1 张阶段摘要；普通卡不删除。
- 恢复普通摘要的纠错标记、锁定状态、单张重新生成。
- 恢复摘要专用 API / 模型选择，沿用会话级 `profileId`、`modelId`。
- 带入对话的普通摘要数量改为可选 3–10 张；同时带入最近 2 张阶段摘要。
- 普通摘要继续采用“麻糍”纯正文 prompt，不恢复已删除的结构化事实/情绪字段。
- 普通摘要与阶段摘要继续随完整会话写入 `/persistent/chat/sessions/*.json`；前端普通摘要和阶段摘要各只显示最近 10 张，更早数据仅隐藏、不删除。
- 旧的 `overview` 阶段摘要兼容迁移为新的纯正文 `content`。

### 验证
- `tsc --noEmit` 通过。
- `git diff --check` 通过。

## 2026-08-08 — 摘要进度、锁定与删除安全修复
- 摘要进度改为根据每张摘要真实保存的 `sourceMessageIds` 计算，不再只看游标；生成成功后立即识别已覆盖楼层，避免同一批 40 轮反复生成相同摘要。
- 增加同步 in-flight ref，摘要请求刚启动但 React state 尚未刷新时也不能重入。
- 自动整理开关现在只控制自动触发；关闭期间的新对话继续计入“尚未整理”，重新开启不会再把整理边界跳到最新消息。
...
## 2026-08-19 — Chat 白屏/缩放、默认会话、重 Roll 与模型删除

### 完成
- Chat 输入框在移动端固定为 16px，并禁用 WebKit 自动文字缩放，避免 iPhone 聚焦后页面被放大/缩小且退出输入后字号异常。
- 历史消息去掉逐条 Framer Motion 包装，只保留分页后的普通 DOM；降低长对话在 iOS PWA/WebView 中的合成层与内存压力，减少系统杀页后白屏重载。
- Chat 首次进入时优先恢复最近更新的非空会话；启动同步从 `/persistent/chat/sessions/` 拉回完整列表后，若本地仍是空白草稿，也会切到最近聊天。用户随后手动切换会话不会被后台同步抢走。
- user 气泡重 Roll 现在始终重新调用模型：已有下一条 assistant 时新增回复版本；若原回复丢失/尚未生成，则创建新的 assistant 消息并立即持久化。
- 模型选择底栏取消默认搜索框和自动弹键盘，打开后直接显示模型列表与 API 分类。
- 模型 API 管理展开后，每个具体模型新增删除按钮；至少保留一个模型。删除正在使用/默认模型时自动选择同 API 的可用模型，配置继续跨设备同步。
- 顺手清理 ChatView 中重复的摘要变量、重复编辑操作栏，并减少消息列表额外间距包装问题。
