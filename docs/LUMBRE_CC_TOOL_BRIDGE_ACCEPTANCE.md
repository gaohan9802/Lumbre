...
当前结论：阶段 5 的本地实现、权限定向测试、完整测试、隔离生产构建和网关镜像构建已通过。Claude Code 可以只通过固定的 Lumbre MCP 请求现有生活工具；其内建 Bash、Shell、文件读写、源码施工、WebSearch 等能力仍全部移除。本阶段没有部署线上、没有读取生产 `/persistent`，也没有使用真实订阅或真实生活数据调用工具。
...
- 尚未用真实 Claude Code 订阅让星星调用一次真实生活工具；本地使用的是结构化假网关/假 MCP 数据。
- 尚未在预览环境验证内部服务 URL、双向 secret、手机确认框、刷新后的工具事件取回和真实图片查看。
...
1. 两个预览服务使用脱敏卷和测试 secret，健康检查确认 `capabilities.lumbreTools: true`。
2. 在一条可丢弃对话切到 CC，让星星调用 `read_period` 或 `read_todo`，确认工具卡与自然回复都出现。
